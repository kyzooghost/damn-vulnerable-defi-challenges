...
function typeHash(primaryType) {
    return ethers.utils.keccak256(ethers.utils.toUtf8Bytes(encodeType(primaryType)));
}
...
function structHash(primaryType, data) {
    return ethers.utils.keccak256(encodeData(primaryType, data));
}
...
const privateKey = ethers.utils.id("cow");
const address = new ethers.utils.SigningKey(privateKey);
console.log(ethers.utils.computePublicKey(privateKey));
//const signature = address.signDigest(signHash());
